...
@section('content')
    ...
    <table class="table table-bordered table-responsive-lg">
        ...
        <tr>
            ...
            <th width="10%" style="text-align: center;">Meeting</th>
        ...
        @foreach ($spents as $spent)
            <tr style="font-size: 13px;">
                <td>{{ ($spent->task)? $spent->task->id : '-' }}</td>
                <td align="center">{{ Carbon\Carbon::parse($spent->date)->format('jS M Y') }}</td>
                <td style="text-align: center;background-color: {{ ($spent->task && $spent->task->user) ? $spent->task->user->bg_color : '#FFFFFF' }};color: {{ ($spent->task && $spent->task->user) ? $spent->task->user->txt_color : '#000000' }}">{{ ($spent->task && $spent->task->user) ? $spent->task->user->name : '-' }}</td>

                <td align="center" style="font-size: 11px">{{ ($spent->task)? $spent->task->project->name : '-' }}</td>
                <td align="center">{{ ($spent->task && $spent->task->meeting == 1)? 'YES' : 'NO' }}</td>
                <td>{{ ($spent->task)? $spent->task->name : '-' }}</td>
                <td align="center">{{ ($spent->task)? $spent->task->estimated_hours : '-' }}</td>
    ...
    {!! $spents->appends($params)->links("pagination::bootstrap-4") !!}
